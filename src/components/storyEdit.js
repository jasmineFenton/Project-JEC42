  ...
  const fetchStory = async () => {
    try {
      ...
      let rawResponse = await fetch("http://localhost:4000/graphql", {
        ...
        body: JSON.stringify({
          query:
            "{story {id, taskName, taskDesc, subTasks, timeRemaining, status}}",
        }),
      });
      ...
      props.stateSetter(`Retrieved ${results.data.story.length} story`);
    } catch (error) {
    ...
    <Card>
      ...
      <CardText>
        ...
        <Table>
          <TableHead>
            <TableRow>
              <TableCell>subtask headers...</TableCell>
            </TableRow>
          </TableHead>
          <TableBody>
